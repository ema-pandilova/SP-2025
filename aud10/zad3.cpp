...
int daliProst(int n, int divisor) {
    //divisor =[2, n-1]
    if (divisor == n) {
        // dokolku divisor stigne do n brojot e prost (ne bil napraven prekin odnosno return 0)
        return 1;
    } else {
        if (n % divisor == 0) {
            // ako najdeme nekoj delitel na brojot vrakjame 0(ne e prost)
            return 0;
        } else {
